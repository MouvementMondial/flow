...
class LaneChangeAccelEnv_speed(Env):
    ...
    * target_velocity: desired velocity for all vehicles in the network, in m/s
    ...
    def compute_reward(self, rl_actions, **kwargs):
        ...
        desired_rl_speed = 30.0
        for veh_id in self.vehicles.get_rl_ids():
            current_speed = self.vehicles.get_speed(veh_id)
            reward = (current_speed/desired_rl_speed)*(current_speed/desired_rl_speed)*(current_speed/desired_rl_speed)
...
class LaneChangeAccelPOEnv_speed(LaneChangeAccelEnv_speed):
